...
RC RecordBasedFileManager::deleteRecord(FileHandle &fileHandle, const vector<Attribute> &recordDescriptor, const RID &rid){
    ...
    SlotDirectoryRecordEntry recordEntry = getSlotDirectoryRecordEntry(pageData, rid.slotNum);
    memmove (
            ((char*)pageData + slotHeader.freeSpaceOffset), 
            ((char*)pageData + slotHeader.freeSpaceOffset - recordEntry.length),
            (recordEntry.offset - slotHeader.freeSpaceOffset + recordEntry.length)
            );

    memmove  (
            ((char*)pageData + sizeof(SlotDirectoryHeader) + (rid.slotNum) * sizeof(SlotDirectoryRecordEntry)),
            ((char*)pageData + sizeof(SlotDirectoryHeader) + (rid.slotNum + 1) * sizeof(SlotDirectoryRecordEntry)),
            ((slotHeader.recordEntriesNumber - rid.slotNum - 1) * sizeof(SlotDirectoryRecordEntry))
            );
...
RC RecordBasedFileManager::updateRecord(FileHandle &fileHandle, const vector<Attribute> &recordDescriptor, const void *data, const RID &rid){
   // deleteRecord(fileHandle, recordDescriptor, rid);
    ...
    if(getPageFreeSpaceSize(pageData) >= sizeof(SlotDirectoryRecordEntry) + recordSize){
        ...
        slotHeader.recordEntriesNumber += 1;
        setSlotDirectoryHeader(pageData, slotHeader);
        //insert slotDirectoryEntry
        SlotDirectoryRecordEntry newRecordEntry;
        newRecordEntry.length = recordSize;
        newRecordEntry.offset = getSlotDirectoryRecordEntry(pageData,rid.slotNum).offset;
        memmove  (
                ((char*)pageData + sizeof(SlotDirectoryHeader) + (rid.slotNum + 1) * sizeof(SlotDirectoryRecordEntry)),
                ((char*)pageData + sizeof(SlotDirectoryHeader) + (rid.slotNum) * sizeof(SlotDirectoryRecordEntry)),
                ((slotHeader.recordEntriesNumber - rid.slotNum) * sizeof(SlotDirectoryRecordEntry))
                );
        setSlotDirectoryRecordEntry(pageData, rid.slotNum, newRecordEntry);
        //add the record data
        memmove  (
                (char*)pageData + slotHeader.freeSpaceOffset - newRecordEntry.length, 
                (char*)pageData + slotHeader.freeSpaceOffset,
                newRecordEntry.offset - slotHeader.freeSpaceOffset + newRecordEntry.length
                );
        setRecordAtOffset (pageData, newRecordEntry.offset, recordDescriptor, data);
        if (fileHandle.writePage(rid.pageNum, pageData)){
            return RBFM_WRITE_FAILED;
        ...
        SlotDirectoryRecordEntry newRecordEntry;
        newRecordEntry.offset = - rid_new.pageNum;
        newRecordEntry.length = rid_new.slotNum;
...
{
    return rbfm_ScanIterator.beginScan(fileHandle, recordDescriptor, conditionAttribute, compOp, value, attributeNames);
}
...
RC RBFM_ScanIterator::beginScan(FileHandle &fh,
...
RC RBFM_ScanIterator::nextSlot()
{
    ...
    if (recordEntry.offset > 0 || !checkScanCondition())
    {
...
bool RBFM_ScanIterator::checkScanCondition()
{
    ...
    else if (attr.type == TypeInt)
    {
        ...
        result = checkInt(recordInt, compOp, value);
    }
    else if (attr.type == TypeReal)
    {
        float recordReal;
        memcpy(&recordReal, (char*)data + 1, REAL_SIZE);
        result = checkFloat(recordReal, compOp, value);
    }
    else if (attr.type == TypeVarChar)
    {
        ...
        result = checkString(recordString, compOp, value);
    }
    free (data);
    return result;
}

bool RBFM_ScanIterator::checkInt(int recordInt, CompOp compOp, const void *value)
{
    int32_t intValue;
...
bool RBFM_ScanIterator::checkFloat(float recordReal, CompOp compOp, const void *value)
{
    float realValue;
...
bool RBFM_ScanIterator::checkString(char *recordString, CompOp compOp, const void *value)
{
    if (compOp == NO_OP)
        return true;
